		...
		this.storage
			...
			.then(res => {
				if (!res || res === 0) {
					...
					this.score = res;
					this.storage.get("history-items").then(res => {
						this.items = res;
					});
					this.storage.get("history-highest").then(res => {
						this.highest = res;
					});
					this.storage.get("historical").then(res => {
						this.historical = res;
					});
				}
			})
		...
		for (let i = 0; i < 4; i++) {
			for (let index = 0; index < this.items.length; index++) {
				...
				if (this.canMoveUp(index)) {
					...
					this.canMergeUp(index) &&
					mergeList.indexOf(index) === -1
				) {
		...
		for (let i = 0; i < 3; i++) {
			for (let index = this.items.length - 1; index >= 0; index--) {
				...
				if (this.canMoveDown(index)) {
					...
					this.canMergeDown(index) &&
					mergeList.indexOf(index + 4) === -1 &&
					mergeList.indexOf(index) === -1
				) {
					mergeList.push(index + 4);
					this.recordScore(item);
			...
			for (let index = this.items.length - 1; index >= 0; index--) {
				...
				if (item !== 0) {
					if (this.canMoveRight(index)) {
						...
						this.canMergeRight(index) &&
						mergeList.indexOf(index) === -1 &&
						mergeList.indexOf(index + 1) === -1
					) {
						mergeList.push(index + 1);
						this.recordScore(item);
						this.items[index + 1] = item * 2;
